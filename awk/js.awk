# Parses the diff of a PR to find the hidden hideous console.log
...
match($0, /^diff \-\-git a\/((.+\/)*(\.?[^\.]+))(\.[a-zA-Z]+)? .*$/, m) { gitFileName = m[3]; if (m[4] != ".js") { gitFileName = m[1] }; } # entering new file
/^@@ \-[0-9]+,[0-9]+ \+[0-9]+,[0-9]+ @@.*$/ { gitLine = gitLineNumber($0); }

/^\+.*console\.log.*$/ { abnormalStuffCounter++; gitPrettyPrint("console.log") }
